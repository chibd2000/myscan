...
class CompanyStructSpider(BaseSpider):
    ...
    async def getDetailInformation(self, url, index, session):
        data = await self.getInformation(url, session)
        self.resList[index]['companyName'] = str(data['companyName'])
    ...
    async def getICPInformation(self, url, index, session):
        data = await self.getInformation(url, session)
        ...
        try:
            async with aiohttp.ClientSession() as session1:
                ...
                for index, value in enumerate(self.resList):
                    ...
                    taskList1.append(self.getDetailInformation(url, index, session1))
        ...
        try:
            async with aiohttp.ClientSession() as session1:
                ...
                for index, value in enumerate(self.resList):
                    ...
                    taskList2.append(self.getICPInformation(url, index, session1))
        ...
        try:
            async with aiohttp.ClientSession() as session1:
                ...
                for index, value in enumerate(self.resList):
                    ...
                    taskList3.append(self.getAppInformation(url, index, session1))
        ...
        try:
            async with aiohttp.ClientSession() as session1:
                ...
                for index, value in enumerate(self.resList):
                    ...
                    taskList4.append(self.getWxInformation(url, index, session1))
    ...
    async def getInformation(self, url, session):
            ...
            async with session.get(url, headers=headers, cookies=cookies) as response:
                if response is not None and response.status == 200:
                    if 'investajax' in url:
                        ...
                        #         'wechatIntruduction': u['wechatIntruduction'],
                        #         'detailUrl': detailUrl
                        #     }
                        #     print(wechatInfo)
                        # self.resList.append(wechatInfo)
                        # self.urlList.append(detailUrl)
                    elif 'company_detail' in url:
    ...
    async def getInvestInformation(self, page, session):
        for _ in range(1, page):
            url = self.investAddr.format(id=self.companyId, page=str(_))
            await self.getInformation(url, session)

    async def getHoldingInformation(self, page, session):
        for _ in range(1, page):
            url = self.holdAddr.format(id=self.companyId, page=str(_))
            await self.getInformation(url, session)

    async def getBranchInformation(self, page, session):
        for _ in range(1, page):
...
if __name__ == '__main__':
    spider = CompanyStructSpider('nbcc.cn', '浙江吉利控股集团有限公司')
