...
class NetSpider(Spider):
    def __init__(self, target, task_queue):
        super().__init__()
        self.headers = {
    ...
    def web_search(self, target):
        web_domain_lists = list()  # 存储爬取到的域名
        ...
        for page in range(1, 10):
            ...
            for i in json_data['results']:
                ...
                sub_domain = i[0]
                self.task_queue.put(sub_domain) # 添加队列中
                ...
                web_domain_lists.append(sub_domain_info)

        web_domain_lists = Common_getUniqueList(web_domain_lists)
        self.write_file(web_domain_lists, target, 3)
        ...
        p = ThreadPoolExecutor(20) # 并发采集，协程8不太会
