...
def Log(x):
    with open('../LangNetWorkTopoLog.txt', 'a+', encoding='utf-8')as a:
        a.write(str('-'.join(
            str(datetime.datetime.now()).replace(' ', '-').replace(':', '-').split('.')[0].split('-'))) + '    ' + str(
            x) + '\n')
...
def Requests(url):
    ...
    if b'66666' not in content2:
        return [{url2: title2}]
    # if title1 != '获取失败' and 'sent to HTTPS port'not in title1 and title2 == '获取失败':
    #     return [{url1: title1}]
    # if title2 != '获取失败' and title1 == '获取失败':
    #     return [{url2: title2}]
    # if title1 != '获取失败' and 'sent to HTTPS port'not in title1 and title2 != '获取失败':
...
class IpInfoScan:
        ...
        if PortInfos != {}:
            for k in list(PortInfos.keys()):
                if PortInfos[k] == '获取失败':
                    ...
                    # 进一步复检，提升准确率
                    if Cpo == True:
                        cks666 = GetPortInfo(str(k)).replace('ms-wbt-server', 'rdp').replace('ms-sql-s', 'mssql')
                        if cks666 != '识别端口异常':
                            PortInfos[k] = cks666.lower()
                        else:
                            chk777 = self.PortLib.get(str(k))
                            if chk777 != None:
                                PortInfos[k] = chk777.lower()
                            else:
                                PortInfos[k] = '指纹库无该端口数据-{}'.format(str(k))
                    else:
                        del PortInfos[k]
    ...
    def FeatureResult(self, openport):
        ...
        for k, v in openport.items():
            ...
            print('[{}]  主机:{} 开放端口:{} 个 部署网站:{} 个 运行服务:{}'.format(str(datetime.datetime.now()).split('.')[0],
                                                                   k.ljust(15),
                                                                   len(list(res.keys())), len(AliveUrls),
                                                                   '/'.join(list(res.values()))))
            Log('主机:{} 开放端口:{} 个 部署网站:{} 个 运行服务:{} 开放端口:{}'.format(k.ljust(15),
                                                                   len(list(res.keys())), len(AliveUrls),
                                                                   '/'.join(list(res.values())), str(list(res.keys()))))
            ...
            if openports != {} and openports != None and openports != []:
                try:
                    ...
                    print('\n[{}]  主机资产:{} 相关端口扫描完毕 发现存活主机共:{} 个 耗时:{}\n'.format(
                        str(datetime.datetime.now()).split('.')[0], self.ip.ljust(15),
                        len(openports), TIMES))
                    time.sleep(2)
                except:
                    pass
...
def CleanData(IPdata, txtfile, htmlfile, Portfolio):
    ...
    with open('../' + htmlfile, 'a+', encoding='utf-8')as a:
        ...
        '''.format(os.path.join(os.path.abspath(''), 'static', 'bootstrap-theme.min.css'),
                   os.path.join(os.path.abspath(''), 'static', 'bootstrap.min.css'),
                   os.path.join(os.path.abspath(''), 'static', 'echarts.min.js'),
                   os.path.join(os.path.abspath(''), 'static', 'echarts-wordcloud.min.js')))
        a.write(ImgData)
        a.write('''
                			<hr />
        			<h1> 主机资产整理表</h1><hr />
        ...
        for file in AllResultFiles:
            a.write('''<a href="{}.txt" target="_blank"><button class="{}">{}</button></a>'''.format(
                os.path.join(os.path.abspath(''), Portfolio, file).replace('/', '\\'), random.choice(Btn_Class), file))
...
if __name__ == '__main__':
    ...
    if res == [] or res == None:
        ...
        print('\n扫描完毕~耗时:{}~发现存活主机总数:{}台\nhtml结果保存在:{}\nxlsx结果保存在:{}'.format(TIME, len(res),
                                                                             os.path.join(os.path.abspath('..'),
                                                                                          ImgHtml),
                                                                             os.path.join(os.path.abspath('..'), Xlsx)))
    while 1:
        time.sleep(500)
