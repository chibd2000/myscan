...
class SubNameBrute(object):
    ...
    def load_sub_names(self):
        ...
        regex_list = []
        lines = set()
        with open(os.path.join("subDomainsBrute" + os.path.sep + self.options.file)) as inFile:
            for line in inFile:
                sub = line.strip()
    ...
    def scan(self, j):
        self.resolvers[j].nameservers = [self.dns_servers[j % self.dns_count]] + self.dns_servers  # resolvers对象
...
def get_sub_file_path():
    if options.full_scan and options.file == 'subnames.txt':
        ...
        if os.path.exists(options.file):
            path = options.file
        elif os.path.exists('subDomainsBrute/dict/%s' % options.file):
...
if __name__ == '__main__':
    ...
    # make tmp dirs
    tmp_dir = 'subDomainsBrute/tmp/%s_%s' % (args[0], int(time.time()))
